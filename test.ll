...
return:                                           ; preds = %if.end19, %if.then17, %2, %if.then
  %retval.0 = phi i32 [ -1, %if.then ], [ -1, %2 ], [ -1, %if.then17 ], [ 0, %if.end19 ]
  ret i32 %retval.0
}
...
return:                                           ; preds = %for.end228, %if.then147, %if.then134, %if.then57, %0, %if.then6, %if.then
  %retval.0 = phi i64 [ -1, %if.then ], [ -1, %if.then6 ], [ -1, %if.then57 ], [ -1, %if.then134 ], [ -1, %if.then147 ], [ 0, %for.end228 ], [ -1, %0 ]
  ret i64 %retval.0
}
...
if.end63:                                         ; preds = %if.end62, %1
  %new_arcs.2 = phi i64 [ %inc57, %1 ], [ %new_arcs.1, %if.end62 ]
  br label %if.end64
...
for.cond:                                         ; preds = %for.inc30, %0
  %block.0 = phi %struct.arc* [ %i4, %0 ], [ %i15, %for.inc30 ]
...
return:                                           ; preds = %3, %if.then72
  %retval.0 = phi %struct.arc* [ null, %if.then72 ], [ %i40, %3 ]
